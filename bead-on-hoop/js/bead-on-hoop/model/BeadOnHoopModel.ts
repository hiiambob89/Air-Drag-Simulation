...
import BeadOnHoopQueryParameters from '../../common/BeadOnHoopQueryParameters.js';
...
export default class BeadOnHoopModel implements TModel {
  ...
  public constructor( providedOptions: BeadOnHoopModelOptions ) {
    ...
    // this.thetaEQ = '\\tan v'
    // if (getUr)
    // console.log(BeadOnHoopQueryParameters.SCHEMA_MAP)
    // console.log()
    if (new URL(window.location.href).searchParams.get("formula") === 'true') {
      ...
      this.thetaEQ = ''
      this.graphDataTest = new simData(this.graphUpdateInterval)
      this.graphDataTest.insert(0,0,0)
