...
export default class BeadOnHoopScreenView extends ScreenView {
  ...
  public constructor(model: BeadOnHoopModel, providedOptions: BeadOnHoopScreenViewOptions) {
    ...
    var textVBoxConstants = new VBox({
      align: 'left', children: [
    ...
    var sliderVboxConstants = new VBox({
      children: [
    ...
    var sliderVboxInitial = new VBox({
      children: [
        new Rectangle(0, 0, 0, 5),
    ...
    var buttonVBoxConstants = new VBox({
      children: [
    ...
    var buttonVBoxInitial = new VBox({
      children: [
        new Rectangle(0, 0, 0, simSpeedSlider.height - exRichText.height + sliderGap.height),
    ...
    var dynamicTextVBoxConstants = new VBox({
      align: 'left', children: [
        new HBox({
          children: [
    ...
    var dynamicTextVBoxInitial = new VBox({
      align: 'left', children: [
        new HBox({
          children: [
            new Rectangle(0, 0, 3, 0),
    ...
    const panelInitial = new Panel(new VBox({ align: 'left', children: [titleInitial, new HBox({ children: [textVBoxInitial, sliderVboxInitial, new HBox({ children: [buttonVBoxInitial] }), new Rectangle(0, 0, 10, 0), dynamicTextVBoxInitial] }), new Rectangle(0, 0, panelWidth, 0)] }), { fill: new Color(210, 210, 210) })
    ...
    const velBox = new HBox({ preferredWidth: 30, children: [velocityField] })
    ...
    const buttonAddThetaDOM = new RectangularToggleButton(buttonAddTheta, false, true, { minWidth: 300, minHeight: 20, content: new RichText('insert (θ) in the equation') })

    const buttonAddOmegaDOM = new RectangularToggleButton(buttonAddOmega, false, true, { minWidth: 300, minHeight: 20, content: new RichText('insert (ω) in the equation') })

    const equationPanel = new Panel(new VBox({
      minimumWidth: 400,
      align: 'center', children: [
        new VBox({ align: "right", children: [buttonAddThetaDOM, new Rectangle(0, 0, 50, 5), buttonAddOmegaDOM,] }),
    ...
    const visualBox = new VBox({
      align: 'left', children: [
        new HBox({
          children: [
            new phet.scenery.Text("simSpeed", { fontSize: 15, textAlign: 'left' }),
            simSpeedSlider,
            new RoundToggleButton(buttonSimSpeed, false, true, { radius: 8 }),
            // new phet.scenery.Text('✎', { fontSize: 15, textAlign: 'center', AlignBox: "center",  }),
            // new RichText('✎'),
            new phet.scenery.Text('✎', { fontSize: 15, textAlign: 'center' }),
            new Rectangle(0, 0, 0, simSpeedSlider.height - exRichText.height + sliderGap.height),
            new HBox({
              children: [
                new Rectangle(0, 0, 10, 0),
                // new RichText(new DerivedProperty([model.simSpeedProp], (value) => { return String(Number(value).toFixed(2)) })),
                new phet.scenery.Text(new DerivedProperty([model.simSpeedProp], (value) => { return String(Number(value).toFixed(2)) }), { fontSize: 15, textAlign: 'left' }),
                 new phet.scenery.Text("x", { fontSize: 15, textAlign: 'left' })
              ]
            }),]
        }),
        new Rectangle(0, 0, 0, 5),
        new HBox({
          children: [
            new phet.scenery.Text("Project trail onto hoop?", { fontSize: 15, textAlign: 'left' }),
    ...
    visualBox.leftTop = new phet.dot.Vector2(this.layoutBounds.maxX / 4.3, resetAllButton.centerTop.y - resetAllButton.height + 30);
